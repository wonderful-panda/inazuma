 ...
 */
pub fn parse_numstat_row(text: &str) -> Result<Vec<FileStat>, GitError> {
    let mut files: HashMap<&str, FileEntry> = HashMap::new();
    let mut ret: Vec<FileStat> = Vec::new();
        ...
        if token.starts_with(":") {
            ...
            match status {
                'M' | 'A' | 'D' | 'T' | 'U' => {
                    ...
                    files.insert(path, FileEntry::new(&path, &status_code, None));
                }
                'R' | 'C' => {
                    let old_path = tokens.pop().unwrap();
                    let path = tokens.pop().unwrap();
                    files.insert(path, FileEntry::new(&path, &status_code, Some(&old_path)));
                }
                _ => {
            ...
            if let Some(file) = files.remove(path) {
                match (values[0].parse::<u32>(), values[1].parse::<u32>()) {
                    (Ok(insertions), Ok(deletions)) => {
                        let delta = FileDelta::Text {
                            insertions,
                            deletions,
                        };
                        ret.push(FileStat::new(file, delta));
                    }
                    _ => {
                        ret.push(FileStat::new(file, FileDelta::Binary));
                    }
                }
            }
        }
...
pub fn get_commit_detail(repo_path: &Path, revspec: &str) -> Result<CommitDetail, GitError> {
