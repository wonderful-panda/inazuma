...
export function tryExec(
  ...
  command: string,
  args: string[],
  stdoutCb: EachLineCallback
): Promise<TryExecResult>;
export function tryExec(
  repoPath: string,
  command: string,
  args: string[]
): Promise<TryExecResultWithStdout>;
export function tryExec(
  repoPath: string,
  command: string,
  args: string[],
  stdoutCb?: EachLineCallback
): Promise<TryExecResult | TryExecResultWithStdout> {
...
export function exec(
  ...
  command: string,
  args: string[]
): Promise<ExecResultWithStdout>;
export function exec(
  repoPath: string,
  command: string,
  args: string[],
  stdoutCb?: EachLineCallback
): Promise<ExecResult | ExecResultWithStdout> {
  const promise = stdoutCb
    ? tryExec(repoPath, command, args, stdoutCb)
    : tryExec(repoPath, command, args);
